...
class WarningRepository
{
    ...
    public function warn(Post $post, array $data)
    {
        $post->afterSave(function (Post $post) use ($data) {
            $warning = new Warning();
            ...
            $warning->severity = $data['severity'] ?? Severity::LOW;
            $warning->expires_at = $data['expires_at'] ?? null;
